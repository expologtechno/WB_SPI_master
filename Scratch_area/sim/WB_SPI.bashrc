...
export AGENT_DIR=$VRF_DIR/agents
export TEST_DIR=$VRF_DIR/tests
export SEQ_DIR=$VRF_DIR/seqs

export LOG_DIR=$PROJ_ROOT/../sim_logs

#DUT FILE
export DUT_FILE="$RTL_DIR/spi_top.v"

#TB TOP FILE
export TOP_FILE="$TB_DIR/tb_top.sv"

#regression tl file name
export TL_FILE="$SIM_DIR/spi_regress"  #NOTE: mention only tl file name : dont not include .tl 

#Include directories for makefile : change this as per your project
export INC_DIR="+incdir+$RTL_DIR +incdir+$AGENT_DIR +incdir+$AGENT_DIR/wb_agent +incdir+$AGENT_DIR/spi_slave_agent +incdir+$AGENT_DIR/reset_agent +incdir+$TEST_DIR +incdir+$SEQ_DIR  +incdir+$TB_DIR +incdir+$LOG_DIR +incdir+$SIM_DIR"
